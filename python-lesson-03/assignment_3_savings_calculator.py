...
interest_rate = float(input("What is the interest rate on this savings account? ").replace("%",""))
    ## I used str.replace to remove any % symbols users may accidentally input. When i was testing this script i kept accidentally inputting the interest rate as 4.5% instead of 4.5.
    ## The extra % symbol could potentially cause issues in the later steps, so the replace function above cleaned that up
    ## I then added some print steps to check if my inputs worked (they did, so i removed those steps from the final code)
...
print()
    ## Added blank print function to make a blank line in the terminal to make my output clearer
